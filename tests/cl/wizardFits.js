...
function rackSizes(hullId, data) {
    const attrs = data.typeDogma[hullId]?.dogmaAttributes ?? [];
    const out = {};
    for (const r of exports.RACKS) {
        out[r] = Math.max(0, Math.round(attrs.find((a) => a.attributeID === RACK_ATTR[r])?.value ?? 0));
    }
    return out;
}
...
function variationEsfFit(fit, v, data) {
    ...
    for (const d of v.drones) {
        const cat = data.types[d.typeId]?.categoryID;
        if (cat !== 18)
            continue; // only real drones fly
        for (let i = 0; i < Math.min(d.qty, 25); i++)
            drones.push({ type_id: d.typeId, state: 'Active' });
    }
    return { ship_type_id: fit.hullId, modules, drones, implants: [] };
}
...
function normalizeFitLayout(fit, sizes) {
    const moved = [];
    let changed = false;
