...
exports.allocateDrones = allocateDrones;
...
function allocateDrones(hullId, bay, data) {
    ...
    for (const item of bay) {
        if (data.types[item.typeId]?.categoryID !== 18)
        ...
        if (benched > 0)
            benchedDrones.push(`${item.name} x${benched}`);
    }
    return {
        drones,
        benchedDrones,
...
function extractStats(result, nonFit, hullOwn = new Set(), benchedDrones = []) {
