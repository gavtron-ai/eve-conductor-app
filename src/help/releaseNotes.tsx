...
export const RELEASE_NOTES: ReleaseNote[] = [
  {
    ...
    changes: [
      ...
      <><b>The app no longer calls br.evetools.</b> Battle Reports used two of that site&apos;s <i>internal</i> routes — found by watching the site&apos;s own traffic — to fetch a fight&apos;s killmails, and, for fights across several systems, to <b>create a saved report on their server by itself</b> when you selected the fight. Nobody there had agreed to that either. Both calls are deleted. The fight summary is now always built from your corporation&apos;s own killmails (zKillboard&apos;s public API and ESI), which the app already holds.</>,
      <><b>What that changes for you:</b> a summary now covers every kill and loss a corp pilot is on. It no longer includes an ally&apos;s loss that nobody in corp got on, or third parties shooting each other in the same fight — the card says “summary from your corp&apos;s own killmails”. The link it copies is still the fight&apos;s public br.evetools related page (an address the app writes out; it contacts nobody), which can only name <b>one system</b>: a multi-system fight&apos;s link covers the first system, and the automatic all-systems saved report is gone. WarBeacon and zKillboard related links are still offered beside it.</>,
      <><b>A false statement corrected.</b> The policy page (⚖ in the header) said of Aperture that “nothing is automated on a timer”. That had not been true for months. The page now says what is true of each service: <b>no contact</b> with Aperture, br.evetools, WarBeacon or EvE-Scout Rescue; zKillboard and Fuzzwork through their public APIs, identified and spaced; CCP through its official API.</>,
      <><b>The Aperture guide shrank with the module.</b> The pages describing the embedded map, its toolbar and the Σ Summary were removed; one page remains, explaining why the module is switched off and what still works. Help text elsewhere that mentioned “pull from Aperture” now describes <b>📋 import map from clipboard</b>.</>,
      <><b>Emergency updates.</b> An update used to install only when the app was quit — and this app is built to be left running in the tray, so an urgent fix could sit unused for days while old copies kept misbehaving. From this version on, a release the author marks as an <b>emergency</b> installs by itself: a red banner says why and counts down one minute, the app closes for a few seconds and comes straight back, and the collectors carry on. An ordinary update behaves as before — a banner, installed when you restart. The feed is now checked hourly instead of every four hours.</>,
    ],
    why: <>The owner, after Aperture&apos;s developer wrote to him: “I want the application to be 100% ethical … get rid of scout rescue interaction and remove capability it enabled … is it more ethical to make the change to the fall back as primary … we should be able to push emergency updates that would occur themselves.”</>,
    policy: <>One rule, applied to every service: a source is sanctioned when its <b>owner</b> has said so — a documented public API is that; a web page or an internal route a tool happens to be able to reach is not, however small the traffic. The emergency channel is deliberately narrow: the only thing the release feed can do is make a published release install sooner. It cannot run code, change settings or switch features off in an installed copy, and it only reaches copies from 0.217.0 on.</>,
    published: true,
  },
  {
