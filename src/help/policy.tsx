...
export const POLICY_THIRD_PARTY: { service: string; rules: ReactNode; how: ReactNode; status: 'clean' | 'note' }[] = [
  ...
  {
    service: 'EvE-Scout Rescue (storm-track page)',
    rules: <>A community site with no API; be a polite reader.</>,
    how: <>One fixed page, fetched at most every 30 minutes, identified. The app reads it and displays it; nothing is written and nothing is republished.</>,
    status: 'clean',
  },
  {
    service: 'br.evetools.org (shareable battle-report links)',
    rules: <>A community battle-report site; its endpoints are its own and may change.</>,
    how: <>Called only when you open or create a report — one request per press, identified. If the site changes or is down the tab says so instead of failing silently.</>,
    status: 'note',
  },
  {
    service: 'Your corporation\'s Aperture map',
    rules: <>Whatever the map's owner allows; it is your own login.</>,
    how: <>Embedded as a normal page with your own session, and read once per press to import your system list. Nothing is automated on a timer; confirm with whoever runs the map that automated reads are fine.</>,
    status: 'note',
  },
  {
