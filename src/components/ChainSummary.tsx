...
import { APERTURE_FEATURES_AVAILABLE, APERTURE_BLOCK_SHORT } from '../lib/apertureAccess';
...
/** v0.215.0: the feature is unavailable (lib/apertureAccess.ts) — every way in shows the blocker */
export default function ChainSummary(props: Parameters<typeof ChainSummaryView>[0]) {
  if (!APERTURE_FEATURES_AVAILABLE) return <ApertureBlocker feature="Σ Summary — what is out there to do in chain" />;
  return <ChainSummaryView {...props} />;
}

export function ChainSummaryView({ embedded = null }: { embedded?: ChainSummaryEmbed | null }) {
    ...
    <div className="chain-root" style={embedded ? { padding: 14, boxSizing: 'border-box' } : { padding: 14, minHeight: '100vh', boxSizing: 'border-box', zoom: zoomLevel }}>
      <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap', marginBottom: 8 }}>
        ...
        <span className="dim" style={{ fontSize: 12 }}>
          {data ? `map read ${data.at ? new Date(data.at).toISOString().slice(11, 16) : ''} EVE · ${parsed?.sigs.length ?? 0} signatures · ${parsed?.nodeCount ?? 0} systems drawn · source: ${parsed?.source ?? '—'}` : !APERTURE_FEATURES_AVAILABLE ? 'no map reading — reading the map is paused' : embedded ? (embedded.busy ? 'reading the map… (the map page is loading; a few seconds)' : 'waiting for the map — log in on the Corp Map tab if it asks, or press ⟳ refresh') :'waiting for the map — press Summary on the Aperture module'}
        </span>
        {!APERTURE_FEATURES_AVAILABLE && <span style={{ fontSize: 12, color: 'var(--warn, #e0a13a)', flexBasis: '100%' }}>⏸ {APERTURE_BLOCK_SHORT}{data ? ' What is below is the last reading taken before the pause.' : ''}</span>}
        {APERTURE_FEATURES_AVAILABLE && stale && <span style={{ fontSize: 12, color: 'var(--warn, #e0a13a)', flexBasis: '100%' }}>⚠ {stale}</span>}
