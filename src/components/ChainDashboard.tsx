...
function ChainMap({ origin, layout, focus, onFocus, offChain, note, routeHome, routeMe, routeNote, routeFrom, originLabel, effectOf, tagOf, clsOf, palette, shattered }: Pick<ChainDashboardProps, 'origin' | 'layout' | 'focus' | 'onFocus' | 'offChain' | 'note' | 'routeHome' | 'routeMe' | 'routeNote' | 'routeFrom' | 'originLabel' | 'effectOf' | 'tagOf' | 'clsOf' | 'palette' | 'shattered'>) {
  const pad = 12;
    ...
    <div className="panel" style={{ padding: 10, flex: '1 1 560px', minWidth: 0 }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 4 }}>
        ...
        <span className="dim" style={{ fontSize: 11 }}
          ...
          {layout.unlinked.length > 0 ? <> · <span style={{ color: '#ff8080' }}>{layout.unlinked.length} on the map but not linked to {origin || 'the origin'}</span></> : null}
...
function HopBars({ bars, maxHops, onMaxHops }: Pick<ChainDashboardProps, 'bars' | 'maxHops' | 'onMaxHops'>) {
  const W = 300, H = 132, padL = 6, padB = 22, padT = 18;
  const max = bars.reduce((m, b) => Math.max(m, b.total), 0) || 1;
  const bw = bars.length ? Math.min(44, (W - padL * 2) / bars.length) : 0;
  return (
    <div className="panel" style={{ padding: 10 }}>
      ...
      {bars.length === 0 ? <div className="dim" style={{ fontSize: 12, padding: 8 }}>no distances yet</div> : (
        <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ display: 'block', maxHeight: 150 }} role="img" aria-label="ISK by hop">
          {bars.map((b, i) => {
            const x = padL + i * bw + 3, w = bw - 6;
            const total = b.total;
            ...
            return (
              <g key={b.hop} style={{ cursor: 'pointer' }} opacity={inRange ? 1 : 0.35} onClick={() => onMaxHops(maxHops === b.hop ? null : b.hop)}>
                ...
                <text x={x + w / 2} y={H - padB - fullH - 4} textAnchor="middle" fontSize={9.5} fill="currentColor" style={{ fontVariantNumeric: 'tabular-nums' }}>{total > 0 ? iskShort(total) : b.count > 0 ? `${b.count}?` : ''}</text>
                <text x={x + w / 2} y={H - 8} textAnchor="middle" fontSize={10} fill={maxHops === b.hop ? 'var(--accent, #7fc8ff)' : DIM} fontWeight={maxHops === b.hop ? 700 : 400}>{b.hop}</text>
              </g>
            );
          })}
        </svg>
...
function ActivityMix({ byGroup }: Pick<ChainDashboardProps, 'byGroup'>) {
  ...
  return (
    <div className="panel" style={{ padding: 10 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '54px 1fr 64px', gap: '4px 8px', alignItems: 'center', marginTop: 6, fontSize: 11.5 }}>
        {GROUP_ORDER.map((g) => {
          ...
          return (
            <div key={g} style={{ display: 'contents' }}>
              ...
              <div style={{ height: 10, background: 'rgba(127,127,127,0.15)', borderRadius: 5, overflow: 'hidden' }} title={`${g}: ${iskShort(t.isk)} · ${t.count} sites${t.unvalued ? ` · ${t.unvalued} without an estimate` : ''}`}>
...
function Freshness({ ages }: Pick<ChainDashboardProps, 'ages'>) {
  ...
  return (
    <div className="panel" style={{ padding: 10 }}>
      ...
      <div style={{ display: 'flex', gap: 4, alignItems: 'flex-end', height: 62, marginTop: 6 }}>
        {ages.map((a) => (
          <div key={a.label} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }} title={`${a.label}: ${a.count} signature${a.count === 1 ? '' : 's'} · ${iskShort(a.isk)}`}>
            <span style={{ fontSize: 9.5, fontVariantNumeric: 'tabular-nums' }}>{a.count || ''}</span>
            <div style={{ width: '100%', height: Math.max(2, Math.round((40 * a.count) / max)), background: a.maxH === null ? LINE : a.maxH <= 3 ? '#8dc169' : a.maxH <= 12 ? '#e0a13a' : '#ff5b5b', borderRadius: 2, opacity: a.count ? 1 : 0.3 }} />
            <span className="dim" style={{ fontSize: 8.5, whiteSpace: 'nowrap' }}>{a.label}</span>
          </div>
        ))}
      </div>
    </div>
...
export default function ChainDashboard(p: ChainDashboardProps) {
  return (
    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 8, alignItems: 'stretch' }}>
      <ChainMap origin={p.origin} layout={p.layout} focus={p.focus} onFocus={p.onFocus} offChain={p.offChain} note={p.note}
      ...
      <div style={{ flex: '1 1 300px', display: 'flex', flexDirection: 'column', gap: 8, minWidth: 280 }}>
