...
export function NoChain() {
  ...
  return <Empty>{!APERTURE_FEATURES_AVAILABLE ? `🚧 ${APERTURE_BLOCK_SHORT}` : url ? 'Waiting for the first map reading — it starts once Aperture has loaded.' : 'Set your corp map’s address in ⚙ Settings → Your setup, then open Aperture once and log in.'}</Empty>;
}
